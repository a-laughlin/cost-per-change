...
const modalOverlay = componentFromStream(pipe(
  ...
  map(C=>C===stubNull?null:createElement(
    Div(styl(`fh posF top0px left0px z999 dB w100% h100%`),pipeClicks(x=>handler(C)))
  ))
));
...
const ModalContainer = componentFromStream(pipe(
  ...
  map(C=>C===stubNull?null:createElement(Div(
    withItems(styl('fh w100% h100% dB')(C)),
    styl(
      `fh posF dB left50% top80px z2000 maxw80% maxh80% p1 bgcF`,
...
const withModal = Component=>compose(pipeClicks(x=>handler(Component)),styl('crP'));
